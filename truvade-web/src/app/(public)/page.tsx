...
function PropertyCardSkeleton() {
  return (
    <div className="min-w-[200px] w-[200px] md:min-w-[240px] md:w-[240px] shrink-0">
...
function CarouselSection({ title, properties, href, loading }: CarouselSectionProps) {
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {href && !loading && (
              <Link
                href={href}
                className="text-sm font-semibold text-gray-900 underline hover:text-gray-600 mr-2"
              >
                Show all
              </Link>
            )}
        ...
        >
          {loading
            ...
            : properties.map((property) => (
                <div
                  key={property.id}
                  className="min-w-[200px] w-[200px] md:min-w-[240px] md:w-[240px] shrink-0"
                >
...
function LocationCardsSection() {
  return (
    <section className="py-6">
      <div className="max-w-[1760px] mx-auto px-3 sm:px-5 lg:px-10">
        <h2 className="text-[22px] font-semibold text-gray-900 mb-4">Top locations</h2>
...
export default function HomePage() {
  ...
  useEffect(() => {
    fetchPublicShortlets({ sort: "newest" })
  ...
  const byState = (state: string) =>
    allProperties.filter((p) => p.state.toLowerCase() === state.toLowerCase());
